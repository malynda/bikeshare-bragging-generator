$(function() {
  ...
  $.ajax({
      ...
      success: function(data) {
        ...
        $.ajax({
          type: "GET",
          url: "http://bikesharebrags-leaderboard.herokuapp.com/entries.json?city=#CITY#",
          success: function(data) {
            leaderboard_html = "";
    ...
    if (start_station !== end_station) {
      ...
      for (k = 0; k < window.lines.length; k++) {
        ...
        if (this_pair["start_station"] === start_station && this_pair["end_station"] === end_station) {
          ...
          // When there are no more trips to calculate, post the results in the notice area of the Bbikesharebrags sidebar
    ...
    $.ajax({
      ...
      success: function(data) {
        if (data.status === "OK") {
          ...
          if (milage < 20) {                            // Sanity check in case Google wildly mis-reads the location of a bikeshare station based on its name.
            window.my_bikeshare_data[i]["milage"] = milage;
            total_milage += milage;
            trips_calculated += 1;
  ...
  function postResults(total_milage) {
    ...
    notice_area_html += "</p><p class='notice-area-text'>Time Riding ${BIKESHARE PROGRAM NAME}: " + window.total_hours + "h, " + window.remainder_minutes + "m, " + window.remainder_seconds + "s</p>";
  ...
  $('#post-to-leaderboard').livequery(function() {
    if (window.username === null || window.username == undefined) {
      $(this).click(function() {
        ...
        enter_leaderboard_name_html += "<br/><a id='post-it' class='bikesharebrags-option'><i>Post to Leaderboard</i></a>";
  ...
  function postIt() {
    ...
    $.ajax({
      type: "POST",
      url: "http://bikesharebrags-leaderboard.herokuapp.com/new_entry",
